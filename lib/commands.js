...
const clattr = {
  handler: (command, author, guildId) => {
    ...
    console.log(`clattr handler got '${subcommand}', '${rest}'`)
    switch (subcommand) {
      case 'p':
      case 'priv':
      case 'private':
...
function handle(message) {
  ...
  try {
    ...
    console.log(`parsing command '${command}' got params '${params}'`)
    if (!commands.has(command)) return false
    const handler = commands.get(command)
    console.log(`found handler ${typeof(handler)}`)
    if (handler) return handler(params, author, guildId)
    return false
  } catch (e) {
